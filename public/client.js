...
const EFFECT_INITIAL_PAUSE_MS = 1800;
const EFFECT_BETWEEN_PAUSE_MS = 1250;
const TURN_CARD_HOLD_MS = 5000;
const CPU_ACTION_HOLD_MS = 3000;
...
function updateChrome() {
  ...
  $('ambienceBtn').title = `Music: ${musicEnabled ? 'on' : 'off'}`;
...
function renderAudioControls() {
  ...
  wrap.innerHTML = `
    ...
    <div class="spotifyBox ${musicSource === 'spotify' ? '' : 'softDisabled'}">
      <h3>Spotify</h3>
      <p>${spotifyEnabled ? 'Connect a Spotify account to use Spotify playback controls. Spotify Web Playback generally requires Premium.' : 'Spotify is not configured on this Render service yet.'}</p>
  ...
  $('spotifyPlayBtn').onclick = async () => { uiClick(); musicSource = 'spotify'; localStorage.setItem('probe_music_source', 'spotify'); musicEnabled = true; localStorage.setItem('probe_music_loop', 'on'); await startSpotifyPlayback(); updateChrome(); renderAudioControls(); };
}
...
function centerTrayPattern(pattern) {
  ...
  if (!clean) return Array.from({ length: 12 }, () => '');
  const left = Math.floor((12 - clean.length) / 2);
  return Array.from({ length: 12 }, (_, i) => clean[i - left] || '');
}

function renderPreviewTray(id = 'self') {
  const tray = ($(`secretWord_${id}`)?.value || '').trim().toUpperCase().replace(/[^A-Z.]/g, '').slice(0, 12);
  const chars = centerTrayPattern(tray);
    ...
    <div class="gameHud">
      ...
      <section class="centerBoard">
        ...
        <div class="boardAndCard ${layoutMode === 'round' ? 'roundLayout' : 'stackedLayout'}">
          <div class="trayTheater ${layoutMode === 'round' ? `circleTheater seatCount${orderedPlayersForBoard().length}` : ''}">${orderedPlayersForBoard().map((p, i, arr) => renderPlayerTray(p, i, arr.length)).join('')}${renderBoardDeckSource()}</div>
          <div class="cardDock">
...
function slotHtml(slot, clickable = false, revealed = false, manual = false) {
  ...
  return `<div class="slotShell" data-slot-index="${slot.index}"><div class="slotValue">${[5,5,5,5,10,10,10,10,15,15,15,15][slot.index]}</div><button class="${classes.join(' ')}" ${attrs}><span class="tileSymbol" aria-hidden="true">✥</span><span class="tileGlyph"><span class="tileGlyphInner">${esc(display)}</span></span></button></div>`;
}
...
function handleGlobalGameKeys(e) {
  ...
  const isEditable = activeEl?.isContentEditable || ['input', 'textarea', 'select', 'button'].includes(tag);
  const controlId = currentControlId();
  const controlled = state.players?.find(p => p.id === controlId);
  ...
  if (e.key === 'Enter') {
    // Inputs already have their own Enter handlers. This catches the common case where the board/body is focused.
    if (!isEditable && input.value.trim()) {
      e.preventDefault();
      askBtn.click();
    }
    return;
...
function fitSlotGlyphs() {
  document.querySelectorAll('.slotTile .tileGlyphInner').forEach(inner => {
    ...
    const maxW = tile.clientWidth * 0.78;
    const maxH = tile.clientHeight * 0.76;
    const w = Math.max(1, inner.scrollWidth);
    const h = Math.max(1, inner.scrollHeight);
    const scale = Math.min(1, maxW / w, maxH / h);
    inner.style.transform = `scale(${Math.max(0.32, scale)})`;
  });
}
...
function showNextEffect() {
  ...
  const t2 = setTimeout(() => { el.remove(); finishEffect(); }, hold + 720);
...
function turnCardInner(effect, label = 'Activity Card') {
  ...
  return `
    <div class="turnCard3d effect_${sentiment} ${code ? cardClass(code) : ''}" aria-live="polite">
...
function showTurnCardEffect(effect, holdMs, done) {
  ...
  // Use the user's card-flip sound at the actual visual flip, then play the card's reward/penalty sound.
  const flipDelay = setTimeout(() => playSoundAsset('flip', [260, 390, 520], 0.24, 0.04, 0.72), 430);
  ...
  const leaveAt = 1350 + holdMs;
  const t1 = setTimeout(() => el.classList.add('turnCardReturning'), leaveAt);
  const t2 = setTimeout(() => { el.remove(); done(); }, leaveAt + 1000);
  effectTimeouts.push(flipDelay, soundDelay, t1, t2);
}
...
async function startSpotifyPlayback() {
  ...
  if (!spotifyPlayer) {
    ...
    spotifyPlayer.addListener('ready', ({ device_id }) => { spotifyDeviceId = device_id; spotifyReady = true; spotifyStatus = 'Spotify player ready'; renderAudioControls(); if (selectedSpotifyPlaylist) playSpotifyContext(selectedSpotifyPlaylist); });
    ...
    spotifyPlayer.addListener('account_error', ({ message }) => { spotifyStatus = `${message} — Spotify Premium is usually required.`; renderAudioControls(); });
    spotifyPlayer.addListener('playback_error', ({ message }) => { spotifyStatus = message; renderAudioControls(); });
  }
  await spotifyPlayer.connect();
  spotifyPlayer.setVolume(clamp(musicVolume, 0, 1)).catch?.(() => {});
  spotifyStatus = spotifyReady ? 'Spotify player ready' : 'Connecting Spotify player...';
  if (selectedSpotifyPlaylist && spotifyDeviceId) await playSpotifyContext(selectedSpotifyPlaylist);
  else if (!selectedSpotifyPlaylist) showToast('Load/select a Spotify playlist in the audio settings.');
  return token;
}
async function playSpotifyContext(uri) {
  ...
  await spotifyApi(`/me/player/play?device_id=${encodeURIComponent(spotifyDeviceId)}`, { method: 'PUT', body: JSON.stringify({ context_uri: uri }) });
