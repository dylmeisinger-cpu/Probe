...
function renderPlayerTray(player, seatIndex = 0, seatTotal = 1) {
  ...
  return `
    <div class="playerRack ${active ? 'activeRack turnOwnerRack' : ''} ${askedTarget ? 'askedTargetRack' : ''} ${player.isCpu ? 'cpuRack' : ''} ${selectable ? 'targetSelectableRack' : ''} ${selected ? 'selectedTargetRack' : ''} seat${seatIndex}" ${selectAttrs} style="--seat:${seatIndex};--seats:${seatTotal};${boardSeatStyle(seatIndex, seatTotal)}">
