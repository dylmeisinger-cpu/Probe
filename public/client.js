...
function showNextEffect() {
  ...
  const hold = effect.type?.startsWith('card') && cardCode !== 'NORMAL' ? 1850 : 1450;
  setTimeout(() => el.classList.add('effectLeaving'), hold);
  setTimeout(() => { el.remove(); showNextEffect(); }, hold + 450);
}
...
function playEffectSound(effect, sentiment) {
  ...
  const code = effect?.meta?.cardCode || '';
  if (sentiment === 'bad') return playTone([180, 140, 95], 0.22, 0.055);
  if (sentiment === 'good') return playTone([392, 523.25, 659.25, 783.99], 0.22, 0.05);
  if (code && code !== 'NORMAL') return playTone([246.94, 329.63, 493.88, 659.25], 0.2, 0.048);
  return playTone([293.66, 392, 523.25], 0.15, 0.038);
}
