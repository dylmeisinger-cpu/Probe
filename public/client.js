...
function renderPlayerTray(player, seatIndex = 0, seatTotal = 1) {
  ...
  const askedTarget = !!(pending && pending.playerId === player.id && player.id !== state.activePlayerId && !hasQueuedResultEffectForTarget(player.id));
...
function showStarterSequenceEffect(effect, done) {
  ...
  const labels = players.map((p, i) => {
    ...
    const labelRot = angle - 90;
    return `<span class="starterWheelName" style="--name-rot:${labelRot}deg;--label-upright:${-labelRot}deg"><em>${esc(p.name)}</em></span>`;
  }).join('');
  const el = document.createElement('div');
