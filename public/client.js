...
function showTurnCardEffect(effect, holdMs, done) {
  ...
  // Hold the readable front, then shrink it back to the board center. The small
  // center card is rendered by boardDeckSource and remains there until the turn changes.
  const shrinkAt = 1650 + Math.max(1200, holdMs);
  const t1 = setTimeout(() => el.classList.add('wv417ShrinkToCenter'), shrinkAt);
  const t2 = setTimeout(() => { el.remove(); done(); }, shrinkAt + 950);
