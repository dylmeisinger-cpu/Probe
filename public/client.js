    ...
    <div class="gameHud">
      ...
      <section class="centerBoard">
        ...
        <div class="boardAndCard ${layoutMode === 'round' ? 'roundLayout' : 'stackedLayout'}">
          <div class="trayTheater ${layoutMode === 'round' ? `circleTheater seatCount${orderedPlayersForBoard().length}` : ''}">${renderBoardPlayersAndDeck()}</div>
          <div class="cardDock">
...
function centerTurnCardMini(card) {
  const ready = card?.id && readyCenterTurnCardId === card.id;
  if (!ready) {
    return `<div class="centerTurnCardMini currentCardWaiting cardBackMini" data-center-card-id="${esc(card?.id || '')}" title="Drawing activity card">
      <div class="miniCardTiny">Drawing</div>
      <strong>WORD<br>VAULT</strong>
      <span>✥</span>
    </div>`;
  }
  return `<div class="centerTurnCardMini currentCardReady ${cardClass(card.code)}" data-center-card-id="${esc(card.id || '')}" title="Current turn card: ${esc(card.title)}">
...
function stackedBoardParts() {
  ...
  return { top: opponents, bottom: self ? [self] : [] };
}
...
function renderBoardPlayersAndDeck() {
  ...
  return `${parts.top.map((p, i, arr) => renderPlayerTray(p, i + 1, arr.length + 1)).join('')}${renderBoardDeckSource()}${parts.bottom.map(p => renderPlayerTray(p, 0, (state?.players || []).length)).join('')}`;
}
...
function showTurnCardEffect(effect, holdMs, done) {
  ...
  const cardId = effect?.meta?.cardId || '';
  if (cardId) {
  ...
  const liftDelay = setTimeout(() => el.classList.add('wv417Lifted'), 120);
  ...
  const shrinkAt = 1850 + Math.max(1200, holdMs);
  const t1 = setTimeout(() => el.classList.add('wv417ShrinkToCenter'), shrinkAt);
  const t2 = setTimeout(() => {
    el.remove();
    if (cardId && state?.currentCard?.id === cardId) {
      readyCenterTurnCardId = cardId;
      animatingTurnCardId = null;
      document.querySelectorAll(`[data-center-card-id="${CSS.escape(cardId)}"]`).forEach(mini => mini.classList.add('currentCardReady'));
    }
    done();
  }, shrinkAt + 950);
