...
function centerTurnCardMini(card) {
  return `<div class="centerTurnCardMini ${cardClass(card.code)}" title="Current turn card: ${esc(card.title)}">
...
function handleGlobalGameKeys(e) {
  ...
  if (e.key === 'Enter') {
    if (!isTypingField && input.value.trim()) {
      e.preventDefault();
      askBtn.click();
    }
    return;
...
function fitSlotGlyphs() {
  document.querySelectorAll('.slotTile .tileGlyphInner').forEach(inner => {
    ...
    inner.style.transform = `translate(-50%, -50%) scale(${Math.max(0.32, scale)})`;
  });
}
...
function showTurnCardEffect(effect, holdMs, done) {
  ...
  const flipDelay = setTimeout(() => {
    el.classList.add('turnCardFaceUp');
  ...
  const leaveAt = 1450 + Math.max(1200, holdMs);
  const t1 = setTimeout(() => el.classList.add('turnCardShrinkingToCenter'), leaveAt);
  const t2 = setTimeout(() => { el.remove(); done(); }, leaveAt + 950);
