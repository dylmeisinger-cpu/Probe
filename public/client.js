...
function findRandomOnlineMatch() {
  const name = getName(true);
  socket.emit('findRandomMatch', { name, token: getOrCreateToken() });
  showToast('Looking for a random online player...');
}

function startDailyPuzzle() {
  const name = getName(true);
  socket.emit('startDailyPuzzle', { name, token: getOrCreateToken() });
  ...
  if (accountUser) {
    panel.innerHTML = `
      <div class="accountSignedIn">
        ...
        <span>${esc(accountUser.email)} ${accountUser.verified ? '• verified' : '• not verified'}</span>
        <div class="buttonRow">
...
function renderDailyHomepage() {
  ...
  panel.innerHTML = `
    ...
    <div class="dailyRulesMini">
      <span>Score starts at ${p?.baseScore || 1200}</span>
      <span>-${p?.guessPenalty || 35} per guess</span>
      <span>-${p?.timePenaltyPer10Sec || 5} per 10s</span>
    </div>
    <button id="dailyPuzzleBtn" class="primaryGameBtn">Play Daily vs Smart CPU</button>
    ${renderDailyLeaderboard(dailyLeaderboard, true)}`;
...
function renderGame() {
  ...
  const isMyTurn = state.activePlayerId === controlId && !controlled?.isCpu && !introPending;
...
function renderPlayerTray(player, seatIndex = 0, seatTotal = 1) {
  ...
  const askedTarget = !!(pending && pending.playerId === player.id && player.id !== state.activePlayerId);
  ...
  return `
    <div class="rackSeatGroup hasRackMediaTile ${active ? 'activeSeatGroup' : ''} ${askedTarget ? 'askedTargetSeatGroup' : ''} ${player.isCpu ? 'cpuSeatGroup' : ''} ${selectable ? 'targetSelectableSeatGroup' : ''} ${selected ? 'selectedTargetSeatGroup' : ''} seat${seatIndex}" style="--seat:${seatIndex};--seats:${seatTotal};${boardSeatStyle(seatIndex, seatTotal)}">
      ...
      <div class="playerRack ${active ? 'activeRack turnOwnerRack' : ''} ${askedTarget ? 'askedTargetRack' : ''} ${player.isCpu ? 'cpuRack' : ''} ${selectable ? 'targetSelectableRack' : ''} ${selected ? 'selectedTargetRack' : ''} seat${seatIndex}" ${selectAttrs}>
...
function clearEndResultsOverlay() {
  ...
  endOverlayKey = '';
  if (victoryAudio) {
    try { victoryAudio.pause(); victoryAudio.currentTime = 0; } catch (_) {}
    victoryAudio = null;
  }
}
  ...
  overlay.innerHTML = `
    ...
    <div class="endResultsCard" role="dialog" aria-modal="true" aria-label="Game results">
      ...
      <div class="buttonRow endResultsActions">
        ${state?.isHost ? '<button id="restartBtn" class="primaryGameBtn">Start Another Game</button>' : '<span class="hint">Waiting for host to start another game.</span>'}
      </div>
    </div>`;
...
function showStarterSequenceEffect(effect, done) {
  ...
  const finalRot = 2160 + (360 - (starterIndex * seg + seg / 2));
  const labels = players.map((p, i) => {
    const angle = i * seg + seg / 2;
    return `<span class="starterWheelName" style="--name-rot:${angle}deg"><em>${esc(p.name)}</em></span>`;
  }).join('');
  const el = document.createElement('div');
  ...
  el.innerHTML = `
    ...
    <div class="starterWheelCard" aria-live="polite">
      ...
      <div class="starterWheelDisc" style="--wheel-bg:conic-gradient(${colors}); --final-rot:${finalRot}deg">
        ${labels}
  ...
  const startWheel = setTimeout(() => {
    ...
    const tickCount = Math.max(18, players.length * 9);
    let ticks = 0;
    tickTimer = setInterval(() => {
      ticks += 1;
      playSoundAsset('starterTick', [760, 540], 0.045, 0.018, 0.28);
      if (ticks >= tickCount) { clearInterval(tickTimer); tickTimer = null; }
    }, Math.max(70, Math.floor(spinMs / tickCount)));
  }, beginMs);
