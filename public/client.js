...
function visualActivePlayerId() {
  if (!state) return '';
  const delayed = effectQueue.find(effect => {
    if (!isResultRevealEffect(effect)) return false;
    const actorId = effect?.meta?.actorId || effect?.meta?.scorerId || '';
    return actorId && actorId !== state.activePlayerId;
  });
  return delayed?.meta?.actorId || delayed?.meta?.scorerId || state.activePlayerId;
}

function hasQueuedResultEffectForTarget(playerId) {
  if (!playerId) return false;
  return effectQueue.some(effect => isResultRevealEffect(effect) && (effect?.meta?.targetId === playerId || effect?.meta?.playerId === playerId));
}
