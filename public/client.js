...
function renderGame() {
  ...
  const isMyTurn = !resultRevealPending && state.activePlayerId === controlId && !controlled?.isCpu && !introPending;
  const visualActiveName = state.players.find(p => p.id === visualActiveId)?.name || state.activePlayerName;
  const visuallyMyTurn = visualActiveId === controlId && !controlled?.isCpu && !introPending;
...
function renderScoreCards() {
  ...
  return `<div class="scoreCards">${sorted.map((p, idx) => `
    <div class="scoreCard ${p.id === visualActiveId ? 'activeScore' : ''}">
...
function renderPlayerTray(player, seatIndex = 0, seatTotal = 1) {
  ...
  const visualActiveId = visualActivePlayerId();
  const visualTargetId = visualResultTargetId();
  ...
  const selected = visualTargetId ? player.id === visualTargetId : (selectable && selectedGuessTargetId && selectedGuessTargetId === player.id);
  ...
  return `
    <div class="rackSeatGroup hasRackMediaTile ${active ? 'activeSeatGroup' : ''} ${askedTarget ? 'askedTargetSeatGroup' : ''} ${player.isCpu ? 'cpuSeatGroup' : ''} ${selectable ? 'targetSelectableSeatGroup' : ''} ${selected ? 'selectedTargetSeatGroup' : ''} seat${seatIndex}" style="--seat:${seatIndex};--seats:${seatTotal};${boardSeatStyle(seatIndex, seatTotal)}">
      ...
      <div class="playerRack ${active ? 'activeRack turnOwnerRack' : ''} ${askedTarget ? 'askedTargetRack' : ''} ${player.isCpu ? 'cpuRack' : ''} ${selectable ? 'targetSelectableRack' : ''} ${selected ? 'selectedTargetRack' : ''} seat${seatIndex}" data-player-id="${esc(player.id)}" ${selectAttrs}>
...
function syncTargetSelectionGlow() {
  ...
  const visualTargetId = visualResultTargetId();
  const displayTargetId = visualTargetId || selectedGuessTargetId;
...
function showNextEffect() {
  ...
  if (isResultRevealEffect(effect)) render();
...
function visualBlockingResultEffect() {
  if (isResultRevealEffect(currentEffect)) return currentEffect;
  return effectQueue.find(effect => isResultRevealEffect(effect)) || null;
}

function visualActivePlayerId() {
  if (!state) return '';
  const delayed = visualBlockingResultEffect();
  const actorId = delayed?.meta?.actorId || delayed?.meta?.scorerId || '';
  return actorId || state.activePlayerId;
}

function visualResultTargetId() {
  const delayed = visualBlockingResultEffect();
  return delayed?.meta?.targetId || delayed?.meta?.playerId || '';
}
