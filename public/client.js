  ...
  panel.innerHTML = `
    <div class="dailyHeroTop">
      <div>
        ...
        <h2>Today&apos;s Vault</h2>
        <p>${p ? esc(p.clue) : 'Play one shared daily puzzle against the clock.'}</p>
      </div>
    ...
    <div class="dailyRulesMini dailyRulesMiniHero">
      <span>No personal tray</span>
      <span>Normal activity cards</span>
      <span>Score falls per guess</span>
      <span>Stopwatch matters</span>
    </div>
    ...
    <div class="dailyBoardFooter">
      <div class="dailyTodayBox">
        ...
        <p>Everyone solves the same word. Fewer guesses and faster solves climb the public board.</p>
      </div>
...
function normalizedThemeModeSetting() {
  const mode = state?.settings?.themeMode;
  if (mode === 'host') return 'database';
  if (['none', 'random', 'database', 'custom'].includes(mode)) return mode;
  return state?.settings?.useThemes ? 'random' : 'none';
}

function renderThemeLobbyControls(themeOptions) {
  const mode = normalizedThemeModeSetting();
  const method = state?.settings?.themeSelectionMethod || 'host';
  if (state?.randomOnline) {
    const label = state.currentTheme ? `${esc(state.currentTheme.emoji || '*')} ${esc(state.currentTheme.name)}` : (state.randomMatchedAt ? 'No Theme' : 'Chosen when matched');
    return `
      <div class="notice themeLobbyNotice">
        <strong>Public matchmaking theme</strong>
        <span>Random Online automatically chooses either No Theme or one random database theme.</span>
        <em>${label}</em>
      </div>`;
  }
  const suggestions = state.themeSuggestions || [];
  const voteCounts = {};
  Object.values(state.themeVotes || {}).forEach(id => { voteCounts[id] = (voteCounts[id] || 0) + 1; });
  const suggestionTools = method !== 'host' && (mode === 'database' || mode === 'custom') ? `
    <div class="themeSuggestionBox">
      <label>Your theme suggestion</label>
      ${mode === 'database'
        ? `<select id="themeSuggestionSelect">${themeOptions}</select>`
        : `<input id="themeSuggestionInput" maxlength="42" placeholder="Custom theme" value="${esc(state.myThemeSuggestion?.name || '')}" />`}
      <button id="submitThemeSuggestionBtn" class="secondaryGameBtn smallBtn">Submit Theme</button>
      <div class="themeSuggestionList">
        ${suggestions.length ? suggestions.map(s => `
          <button class="themeVoteOption ${state.myThemeVote === s.id ? 'selectedThemeVote' : ''}" data-theme-vote="${esc(s.id)}" ${method === 'vote' ? '' : 'disabled'}>
            <strong>${esc(s.emoji || '*')} ${esc(s.name)}</strong>
            <span>${esc(s.playerName || 'Player')}${method === 'vote' ? ` • ${voteCounts[s.id] || 0} votes` : ''}</span>
          </button>`).join('') : '<p class="hint">No suggestions yet.</p>'}
      </div>
    </div>` : '';
  return `
    <label>Theme mode</label>
    <select id="themeModeSelect" ${state.isHost ? '' : 'disabled'}>
      <option value="none" ${mode === 'none' ? 'selected' : ''}>No Theme</option>
      <option value="random" ${mode === 'random' ? 'selected' : ''}>Random Theme</option>
      <option value="database" ${mode === 'database' ? 'selected' : ''}>Choose From Database</option>
      <option value="custom" ${mode === 'custom' ? 'selected' : ''}>Custom Theme</option>
    </select>
    <label>Theme selection method</label>
    <select id="themeSelectionMethodSelect" ${state.isHost && (mode === 'database' || mode === 'custom') ? '' : 'disabled'}>
      <option value="host" ${method === 'host' ? 'selected' : ''}>Host Chooses</option>
      <option value="vote" ${method === 'vote' ? 'selected' : ''}>Player Voting</option>
      <option value="random" ${method === 'random' ? 'selected' : ''}>Random Selection</option>
    </select>
    <div class="themeSettingGrid">
      <select id="themeSelect" ${state.isHost && mode === 'database' && method === 'host' ? '' : 'disabled'}>${themeOptions}</select>
      <input id="customThemeInput" maxlength="42" placeholder="Custom theme" value="${esc(state.settings.customThemeName || '')}" ${state.isHost && mode === 'custom' && method === 'host' ? '' : 'disabled'} />
    </div>
    <p class="hint">Themes are prompts, not strict validation. Random Online does not allow custom or specific theme picks.</p>
    ${suggestionTools}`;
}
...
function renderLobby() {
      ...
      <div class="grid twoUp">
        ...
        <div class="woodPanel settingsPanel">
          ...
          <select id="timerSelect" ${state.isHost ? '' : 'disabled'}>
            ${[0, 30, 40, 45, 60, 90, 120].map(v => `<option value="${v}" ${state.settings.turnTimerSec === v ? 'selected' : ''}>${v === 0 ? 'Off' : `${v} seconds`}</option>`).join('')}
          </select>

          ${renderThemeLobbyControls(themeOptions)}
  ...
  if (state.isHost) {
    ...
    const sendSettings = () => socket.emit('setSettings', {
      ...
      themeMode: $('themeModeSelect')?.value || 'none',
      themeSelectionMethod: $('themeSelectionMethodSelect')?.value || 'host',
      themeId: $('themeSelect')?.value || state.settings.themeId,
      customThemeName: $('customThemeInput')?.value || '',
      sharedDevice: $('sharedToggle').checked,
      manualReveal: $('manualToggle').checked
    });
    ['timerSelect','themeModeSelect','themeSelectionMethodSelect','themeSelect','customThemeInput','sharedToggle','manualToggle'].forEach(id => { const el = $(id); if (el) el.onchange = () => { uiClick(); sendSettings(); }; });
  }
  if ($('submitThemeSuggestionBtn')) $('submitThemeSuggestionBtn').onclick = () => {
    uiClick();
    socket.emit('submitThemeSuggestion', {
      themeId: $('themeSuggestionSelect')?.value || $('themeSelect')?.value,
      customThemeName: $('themeSuggestionInput')?.value || $('customThemeInput')?.value || ''
    });
  };
  document.querySelectorAll('[data-theme-vote]').forEach(btn => btn.onclick = () => { uiClick(); socket.emit('voteThemeSuggestion', { suggestionId: btn.dataset.themeVote }); });
...
function renderRandomOnlineLobbyCard() {
  ...
  return `
    <div class="notice randomOnlineNotice">
      ...
      ${state.randomRealPlayers >= 2 ? '<em>Tab-away rule: if you leave the tab or screen for 20 seconds during play, you lose 20 points and your turn.</em>' : ''}
      ${cpuButton}
    </div>`;
}
...
function themeBanner() {
  ...
  const examples = (state.currentTheme.examples || []).filter(Boolean).slice(0, 5);
  const detail = examples.length
    ? `Examples: ${examples.map(esc).join(', ')}`
    : 'Broad prompt for the round. It should guide the vibe, not trap you into one tiny word list.';
  return `<div class="themeBanner"><strong>${esc(state.currentTheme.emoji)} Theme: ${esc(state.currentTheme.name)}</strong><span>${detail}</span></div>`;
}
...
function secretFormHtml(id) {
  return `
    <div class="secretForm" data-secret-id="${esc(id)}">
      ...
      <input id="secretWord_${esc(id)}" name="word_vault_secret_tray_${esc(id)}" maxlength="12" autocomplete="off" autocorrect="off" autocapitalize="characters" spellcheck="false" data-lpignore="true" data-1p-ignore="true" placeholder="Example: .TREASURE. or TREASURE" inputmode="text" />
...
function renderGame() {
  ...
  $('game').innerHTML = `
    <div class="gameHud ${state.dailyPuzzle ? 'dailyGameHud' : ''} ${phoneLayout ? 'phoneGameHud' : ''} ${isMyTurn ? 'phoneYourTurn' : 'phoneOtherTurn'} ${waitingOnControl ? 'phoneWaitingReveal' : ''}">
...
function renderPreGameRulesOverlay() {
  ...
  if (state.dailyPuzzle) {
    ...
    return `
    <div class="rulesIntroOverlay dailyRulesIntroOverlay" role="dialog" aria-modal="true" aria-labelledby="rulesIntroTitle">
      <div class="rulesIntroCard dailyRulesIntroCard">
        <div class="tinyCaps">Daily Puzzle</div>
        <h2 id="rulesIntroTitle" class="rulesIntroTitle">Solve Wolt&apos;s Vault</h2>
        <p>${esc(info.clue || 'One shared puzzle for everyone today. The stopwatch starts after this screen.')}</p>
        <ul class="rulesIntroList">
          <li>Your secret tray is not used in Daily mode.</li>
          <li>Choose Wolt and guess letters or dots to uncover the hidden word.</li>
          <li>Activity cards still appear, so wait for each card to finish before acting.</li>
          <li>Full-word guesses can finish the puzzle when you know it.</li>
          <li>Your score starts at ${Number(info.baseScore || 1200)} and drops by ${Number(info.guessPenalty || 35)} per guess.</li>
          <li>The stopwatch trims ${Number(info.timePenaltyPer10Sec || 5)} points every 10 seconds.</li>
          <li>Best scores and fastest times land on today&apos;s public leaderboard.</li>
        </ul>
  ...
  return `
    <div class="rulesIntroOverlay" role="dialog" aria-modal="true" aria-labelledby="rulesIntroTitle">
      <div class="rulesIntroCard">
        <div class="tinyCaps">Word Vault v4.64</div>
...
function visualBoardTargetId() {
  if (starterIntroVisualLockActive()) return '';
...
function renderBoardFullscreenTools(isMyTurn, waitingOnMe, controlled, turnCardReady = currentTurnCardReady(), pending = visibleAwaitingExpose()) {
  ...
  } else if (state?.challengeOffer) {
    const mine = state.challengeOffer.playerId === state.youId || (state.isHost && state.settings.sharedDevice && canControlTray(state.challengeOffer.playerId));
    drawerBody = `<div class="notice">1v1 Challenge: ${esc(state.challengeOffer.playerName)} must choose whether to continue.</div>${mine ? '<div class="buttonRow"><button id="challengeAcceptBtn" class="primaryGameBtn">Accept Defeat</button><button id="challengeContinueBtn" class="purpleBtn">Continue</button></div>' : ''}`;
...
function renderActionPanel(isMyTurn, waitingOnMe, controlled, turnCardReady = currentTurnCardReady(), pending = visibleAwaitingExpose()) {
  if (state?.rulesIntroPending) {
    const text = state.dailyPuzzle
      ? (state.isHost ? 'Review the Daily rules on the board, then press Got it to start the stopwatch.' : 'Waiting for the host to start the Daily Puzzle.')
      : (state.isHost ? 'Review the quick rules on the board, then press Got it.' : 'Waiting for the host to continue.');
  ...
  if (state?.challengeOffer) {
    const mine = state.challengeOffer.playerId === state.youId || (state.isHost && state.settings.sharedDevice && canControlTray(state.challengeOffer.playerId));
    return `
      <div class="commandTitle">1v1 Challenge</div>
      <div class="notice">${esc(state.challengeOffer.playerName)} can accept defeat or keep trying to solve ${esc(state.challengeOffer.targetName)}.</div>
      ${mine ? '<div class="buttonRow"><button id="challengeAcceptBtn" class="primaryGameBtn">Accept Defeat</button><button id="challengeContinueBtn" class="purpleBtn">Continue Challenging</button></div>' : '<p class="hint">Waiting for their decision.</p>'}`;
  }
  ...
  return `
    <div class="commandTitle">${state.challengeMode ? `Challenge ${Number(state.challengeMode.multiplier || 1).toFixed(1)}x` : (state.settings.sharedDevice && state.isHost ? `${esc(controlled.name)} Controls` : 'Make a Guess')}</div>
    ${state.challengeMode ? `<div class="notice">10 seconds per guess. Target: ${esc(state.challengeMode.targetName)}.</div>` : ''}
...
function renderDailyGamePanel() {
  ...
  return `
    ...
    <div class="dailyModeNote"><strong>No personal tray</strong><span>Daily is all about solving Wolt&apos;s hidden word with the normal turn-card effects in play.</span></div>
...
function orderedPlayersForBoard() {
  if (state?.dailyPuzzle) return (state.players || []).filter(p => p.isCpu);
...
function stackedBoardParts() {
  ...
  if (state?.dailyPuzzle) return { top: players.filter(p => p.isCpu), bottom: [] };
...
function attachActionHandlers(controlId) {
  ...
  document.querySelectorAll('#challengeAcceptBtn').forEach(btn => btn.onclick = () => { uiClick(); socket.emit('challengeDecision', { continueChallenge: false }); });
  document.querySelectorAll('#challengeContinueBtn').forEach(btn => btn.onclick = () => { uiClick(); socket.emit('challengeDecision', { continueChallenge: true }); });
...
function showNextEffect() {
  ...
  if (effect?.type === 'theme-reveal') {
    showThemeRevealEffect(effect, finishEffect);
    return;
  }
...
function starterIntroVisualLockActive() {
  return !!(state?.startingIntro || currentEffect?.type === 'starter-sequence' || effectQueue.some(effect => effect?.type === 'starter-sequence'));
}
...
function showThemeRevealEffect(effect, done) {
  const layer = ensureEffectLayer();
  const theme = effect?.meta?.theme || null;
  const noTheme = !!effect?.meta?.noTheme || !theme;
  const el = document.createElement('div');
  el.className = 'themeRevealOverlay';
  el.innerHTML = `
    <div class="themeRevealVault">
      <div class="themeVaultDoor"><span></span></div>
      <div class="tinyCaps">Round Theme</div>
      <h2>${noTheme ? 'No Theme' : `${esc(theme.emoji || '*')} ${esc(theme.name || 'Theme')}`}</h2>
      <p>${noTheme ? 'Original Word Vault rules. No category prompt this round.' : `Examples: ${(theme.examples || []).slice(0, 5).map(esc).join(', ')}`}</p>
    </div>`;
  layer.appendChild(el);
  playTone([196, 246.94, 329.63, 392], 0.32, 0.04);
  const t1 = setTimeout(() => el.classList.add('themeRevealOpen'), 120);
  const t2 = setTimeout(() => el.classList.add('effectLeaving'), 2500);
  const t3 = setTimeout(() => { el.remove(); done(); }, 3050);
  effectTimeouts.push(t1, t2, t3);
}
...
function showStarterSequenceEffect(effect, done) {
  ...
  const starterCenter = starterIndex * seg + seg / 2 + wheelOffset;
  const finalRot = 2160 - starterCenter;
  const labels = players.map((p, i) => {
    const angle = i * seg + seg / 2 + wheelOffset;
    const labelWidth = Math.max(46, Math.min(92, Math.floor((Math.PI * 170 / Math.max(1, players.length)) * 0.74)));
    return `<span class="starterWheelSegmentName" style="--segment-rot:${angle}deg;--label-counter:${-angle}deg;--label-counter-final:${-(angle + finalRot)}deg;--label-width:${labelWidth}px"><em>${esc(p.name)}</em></span>`;
  }).join('');
  const el = document.createElement('div');
  ...
  el.innerHTML = `
    ...
    <div class="starterWheelCard" aria-live="polite">
      ...
      <div class="starterWheelDisc" style="--wheel-bg:conic-gradient(from ${wheelOffset}deg, ${colors});--wheel-lines:conic-gradient(from ${wheelOffset}deg, ${lines});--final-rot:${finalRot}deg">
        ${labels}
